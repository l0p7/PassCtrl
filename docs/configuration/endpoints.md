...
| `server.variables.secrets` | Docker/Kubernetes secrets loaded from `/run/secrets/` at startup and exposed as `variables.secrets.*` in CEL and templates. Uses null-copy semantics. Trailing newlines automatically trimmed. | Loaded secrets can influence backend requests (e.g., API keys, database passwords) via CEL conditions and variable exports. | Secrets can appear in rendered responses when used in templates (use caution to avoid leaking sensitive data). |
